...
<?php foreach ($answers as $answer) { ?>
    ...
    <li>
        <h3 id="a<?= $answer->id ?>"><?= $accepted ?>Skrivet av: <a href="<?= url("user/view?user=")?><?= $answer->author ?>"><?= $answer->author ?><img src="https://www.gravatar.com/avatar/<?= md5($answer->email) ?>?d=robohash&s=40" alt="robot"></a></a>, <small><i><?= $answer->date ?></i></small>
    ...
    <?php foreach ($comments as $comment) { ?>
        <?php if ($comment->answer == $answer->id) : ?>
            ...
            <li>
            <h4 id="c<?= $comment->id ?>">Kommentar av: <a href="<?= url("user/view?user=")?><?= $comment->author ?>" ><?= $comment->author ?><img src="https://www.gravatar.com/avatar/<?= md5($comment->email) ?>?d=robohash&s=40" alt="robot"></a></a>
            <small style="float: right;">
